...
switch($option){
    ...
    case "registrar_itemserv":
        registrar_itemserv();
        break;
    case "editar_itemserv":
        editar_itemserv();
        break;
    case "eliminar_itemserv":
        eliminar_itemserv();
        break;
    case "actualizar_itemserv":
        actualizar_itemserv();
        break;
    case "listar_itemservs":
        listar_itemservs();
        break;
...
/*************************************************************************************************************/
/*************************************************************************************************************/
function registrar_itemserv(){
    $itemserv = new Itemserv;
    $itemserv->storeFormValues($_POST);
    $iditemserv = $itemserv->insert();
    echo json_encode(array("itemserv_id"=>$iditemserv));
}
function eliminar_itemserv(){
    $itemserv = new Itemserv;
    $itemserv->storeFormValues($_POST);
    echo json_encode(array("success"=>$itemserv->delete()));
}
function editar_itemserv(){
    $iditemserv = $_POST["itemserv_id"];
    $itemserv = Itemserv::getById($iditemserv);
    echo json_encode(array("itemserv"=>$itemserv));
}
function actualizar_itemserv(){
    $iditemserv = $_POST["itemserv_id"];
    $itemserv = Itemserv::getById($iditemserv);
    $itemserv->storeFormValues($_POST);
    echo json_encode(array("success"=>$itemserv->update()));
}
function listar_itemservs(){
    $start = $_GET['iDisplayStart']*1;
    $limit = $_GET['iDisplayLength']*1;
    $aColumns = array("itemserv_id", "itemserv_text", "servicio_id");
    $whereParams = array();
    $orderParams = array();
    if ( isset( $_GET['iSortCol_0'] ) ){
        for ( $i=0 ; $i<intval( $_GET['iSortingCols'] ) ; $i++ ){
    ...
    $itemservs = Itemserv::getByFields($whereParams, $orderParams, $start, $limit);

    echo json_encode(array(
        "sEcho" => intval($_GET['sEcho']),
        "iTotalRecords" => $itemservs["totalCount"]*1,
        "iTotalDisplayRecords" => $itemservs["totalCount"]*1,
        "aaData" => $itemservs["itemservs"]
    ));
}
